...
- 一次会话内 T 恒定。浏览器默认 **512**（整帧 576B → QR V16 81×81）；核心库默认 **1024**（整帧 1088B → QR V23 109×109）。接收端从帧头 `symbol_size` 自适应。
...
`scripts/build-all.sh` 的 release 打包文件名也含版本号（硬编码 `v1.0.0`），发布时一并改。
...
| 默认 symbol_size（浏览器/核心库） | 512 / 1024 | `compress.rs` / `config.rs:8` |
