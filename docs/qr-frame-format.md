...
| 1400（浏览器默认，激进预设） | 1404 | V25 | 117×117 |
| 1024（核心库默认） | 1088 | V23 | 109×109 |

### 速度预设（浏览器发送端）

UI 把 (symbol_size, fps) 打包成一个用户选项；默认「激进」实测最快：

| 预设 | symbol_size | 推荐 fps | 版本 | 说明 |
|------|-------------|---------|------|------|
| 稳定 | 512 | 45 | V16 | 最易扫，弱环境/老设备 |
| 高速 | 896 | 60 | V21 | 平衡 |
| 极限 | 1008 | 60 | V22 | 更密、对焦要求更高 |
| 激进（默认） | 1400 | 60 | V25 | 实测最快；码密度高，对对焦/距离要求严格 |
