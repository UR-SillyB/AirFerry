...
`symbol_size` 可配置：浏览器发送端默认 **1400 字节**（激进预设，整帧 1404B → QR V25 117×117），核心库默认 1024 字节（整帧 1088B → QR V23 109×109）。接收端从帧头的 `symbol_size` 字段读取并自适应，因此一次会话内所有帧大小一致。浏览器端的速度预设提供 512 / 896 / 1008 / 1400 四档（见 [二维码帧格式](qr-frame-format.md)）。
